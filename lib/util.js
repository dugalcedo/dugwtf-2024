...
export function serverError(req, res, error, options) {
    let info, userMsg
...
export const caseInsCompare = (s1, s2) => s1.toLowerCase() == s2.toLowerCase()
export const caseInsStartsWith = (s1, s2) => s1.toLowerCase().startsWith(s2.toLowerCase())
export const caseInsIncludes = (arr, s) => arr.filter(x => typeof x == 'string').map(s => s.toLowerCase()).includes(s.toLowerCase())
export function toDugId(s) {
